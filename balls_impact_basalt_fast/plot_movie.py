...
for frame_idx, particle_file in enumerate(particle_files):
    ...
    time_step_size = 0.00005 # assuming constant time step size; adjust as needed
    ...
    ax.set_title(f'Particle velocity at t = {frame_idx * time_step_size:.3f} s', fontsize=24)
    ax.set_xlabel('X (m)', fontsize=28, labelpad=22)
    ax.set_ylabel('Y (m)', fontsize=28, labelpad=22)
    ax.set_zlabel('Z (m)', fontsize=28, labelpad=22)
    ...
    cbar = plt.colorbar(sc, shrink=0.6, pad=0.12)
    cbar.set_label(color_label, fontsize=28, labelpad=15)
    cbar.ax.tick_params(labelsize=24)
    plt.tight_layout()
    frame_filename = os.path.join(output_dir, f'frame_{frame_idx:04d}.png')
    plt.savefig(frame_filename, dpi=100, bbox_inches='tight', pad_inches=0.25)
    plt.close(fig)
# optimize the png
