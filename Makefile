...
CUDA_DIR   ?= $(if $(CONDA_PREFIX),$(CONDA_PREFIX),/usr/local/cuda)
...
LDFLAGS      += -Xlinker -rpath -Xlinker $(CUDA_LIB)/lib -Xlinker -rpath -Xlinker $(CUDA_LIB)/lib64 -ccbin ${CC} -L$(CUDA_LIB)/lib -L$(CUDA_LIB)/lib64 -L/usr/lib/x86_64-linux-gnu/hdf5/serial -lcudart -lpthread -lconfig -lhdf5
