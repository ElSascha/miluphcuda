...
for frame_idx, particle_file in enumerate(particle_files):
    ...
    ax.set_zlabel('Z (m)', fontsize=45, labelpad=60)

    ax.yaxis.get_offset_text().set_fontsize(16)
    ax.tick_params(axis='both', which='major', labelsize=35, pad=25)
    
    # Set explicit ticks that avoid the very edges to prevent overlap
    # Reduce number of ticks to prevent overlap with large font sizes
    ticks = [0, 0.5, 1.0]
    ax.set_xticks(ticks)
    ax.set_yticks(ticks)
    ax.set_zticks(ticks)
    ...
    cbar = plt.colorbar(sc, shrink=0.6, pad=0.2)
    cbar.set_label(color_label, fontsize=45, labelpad=15)
