...
AGGREGATE_RADIUS = 0.01 # 1 cm
POINT_DISTANCE = AGGREGATE_RADIUS / 6 # effect: 4 shells (but still ~isotropic distribution)
NR_INTERACTIONS_NORMAL = 50 # in the middle of the aggregate, expect N interaction partners (10% of max=512)
SMOOTHING_LENGTH = POINT_DISTANCE * 3.8 # Factor 1.866 for Wendland C6 kernel to get ~100 neighbors
# start off with smoothing lengths almost touching the other aggregate,
# such that it crosses during the simulation (t_total = 0.05 s, so with v = 0.01: d = 0.0005)
AGGREGATE_SHIFT = AGGREGATE_RADIUS + SMOOTHING_LENGTH / 2 + 0.0005 / 4

print(AGGREGATE_RADIUS, AGGREGATE_SHIFT, SMOOTHING_LENGTH, POINT_DISTANCE)

DISTENTION = 1.00
RHO_0_RUBBER = 1.0
RHO_0_IMPACT = 2.7e3
RHO_0 = RHO_0_IMPACT # monomer material density
...
# output
#print(p.shape)
np.savetxt('data/stable_ball.0000', p.T, delimiter='\t')
