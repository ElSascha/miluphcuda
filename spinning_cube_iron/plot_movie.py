...
for frame_idx, particle_file in enumerate(particle_files):
    ...
    # Increased font sizes for small subplot usage
    ax.set_title(f'Particle velocity at t = {frame_idx * time_step_size:.3f} s', fontsize=30)
    ax.set_xlabel('X (m)', fontsize=25, labelpad=20)
    ax.set_ylabel('Y (m)', fontsize=25, labelpad=20)
    ax.set_zlabel('Z (m)', fontsize=25, labelpad=20)
    
    ax.tick_params(axis='both', which='major', labelsize=20, pad=5)
    ...
    cbar.set_label(color_label, fontsize=25, labelpad=15)
    cbar.ax.tick_params(labelsize=20)
    
    # Adjust layout manually to prevent cutting off large labels
    plt.subplots_adjust(left=0.15, right=0.9, bottom=0.15, top=0.9)
    
    frame_filename = os.path.join(output_dir, f'frame_{frame_idx:04d}.png')
    # Use bbox_inches='tight' with padding to ensure everything is saved
    plt.savefig(frame_filename, dpi=100, bbox_inches='tight', pad_inches=0.3)
    plt.close(fig)
