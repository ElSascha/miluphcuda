...
particle_files = sorted([f for f in os.listdir(data_dir) if f.startswith('stable_ball.') and f.endswith('.h5')])
...
for frame_idx, particle_file in enumerate(particle_files):
    ...
    ax.set_title(f'Particle velocity at t = {frame_idx * time_step_size:.3f} s')
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Z (m)')
    ...
    ax.set_box_aspect([2,2,2])  # Set aspect ratio to be equal
